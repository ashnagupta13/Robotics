...
==============================================================================
Remarks:
      skeleton to create the sample task
============================================================================*/
...
static double HFE = 0.35;
static double KFE = 0.5;
static int cog_left = 0;
...
static void MoveLeg(void) {
  ...
  if (cog_left == 0) {
    target[L_HFE].th += temp_HFE;
    target[L_KFE].th += temp_HFE;
  } else {
    target[R_HFE].th += temp_HFE + 0.05;
    target[R_KFE].th += temp_KFE;
  }
}
...
run_balance_task(void)
{
  ...
  case ASSIGN_COG_TARGET:
    ...
    if (cog_left == 0) {
      cog_target.x[_X_] = cart_des_state[RIGHT_FOOT].x[_X_]-0.01;
      cog_target.x[_Y_] = cart_des_state[RIGHT_FOOT].x[_Y_]+0.025;
      cog_target.x[_Z_] = 0;//?
    } 
    else
    {
      cog_target.x[_X_] = cart_des_state[LEFT_FOOT].x[_X_]+0.015;
      cog_target.x[_Y_] = cart_des_state[LEFT_FOOT].x[_Y_]+0.025;
      cog_target.x[_Z_] = 0;//?
    }
  ...
  case MOVE_TO_COG_TARGET: // this is for inverse kinematics control
    ...
    //[ thd_des xd_base_des, w_base_des] = J^# * cog_ref.xd
    for (i=1; i<=N_DOFS; ++i) {
